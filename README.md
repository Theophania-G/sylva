...
- **桌面小组件（插件实例）**：控制中心「组件」页点「添加待办事项 / 添加便签」，桌面上就新多出一张可拖动、可缩放、可关闭的小卡片；每个小组件数据独立（待办可勾选/删除/回车添加，便签多行随手记失焦自动保存），旧的 `desk.json` 待办会自动迁移成一个待办小组件。
- **控制中心**：可折叠胶囊 ⇄ 展开面板，两个标签页：
  - **组件**：已添加的小组件列表 + 「＋ 添加待办事项」「＋ 添加便签」；
  - **栅栏管理**：每栅栏的布局（网格/列表）、图标大小、背景风格（玻璃/描边/纯色）、色调色板一键调整，实时生效并持久化；
  - 标题栏「切换桌面」按钮：一键在栅栏接管与原始桌面之间切换（栅栏与小组件淡入淡出，控制中心保持可见以便切回）。
- **D2D 内联文本编辑**：所有输入（待办添加、便签、重命名栅栏/图标）直接在合成表面内绘制与编辑，无悬浮黑框、文字与圆角输入框天然对齐；键盘/中文 IME（拼音候选跟随光标）/剪贴板粘贴 / 光标方向键全部可用。
- **动效**：栅栏拖动 / 缩放丝滑跟随（过冲回弹）、面板展开过冲、图标悬停放大 + 柔光、小组件关闭淡出、待办行入场 / 勾选动画；空闲 0% CPU。
